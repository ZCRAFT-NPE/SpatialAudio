...
import static me.zcraft.mods.spatialaudio.config.BlueTapePack.mixin.GuiRegistriesAccessorMixin.getGuiRegistries;
...
public class GuiRegistryinit {
    ...
    public static void register() {
        ...
        registry.registerPredicateProvider((i13n, field, config, defaults, registry1) -> {
            ...
            return Collections.singletonList(new NestedListListEntry(Component.translatable(i13n), configValue, false, (Supplier)null, (abstractConfigListEntries) -> {
            }, () -> {
                Map ll = (Map<Object, Object>) Utils.getUnsafely(field, defaults);
                return ll == null ? List.of() : new ArrayList<>(ll.values());
            }, Component.literal(""), false, true, (elem, nestedListListEntry) -> {
                if (elem == null) {
